...
public class Anti4D4V extends JavaPlugin implements CommandExecutor, TabCompleter {
    ...
    public boolean hasExemptPermission(UUID uuid) {
        ...
        try {
            ...
            return user != null
                    && user.getCachedData().getPermissionData().checkPermission("anti4d4v.exempt").asBoolean();
        } catch (Exception e) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "iprange":
                return handleIpRangeCommand(sender, args);
            default:
                sendHelpMessage(sender);
                return true;
        }
    ...
    private boolean handleActionCommand(CommandSender sender, String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            for (String value : args[i].split(",")) {
                String action = value.trim().toUpperCase();
        ...
        List<String> validActions = Arrays.asList("BAN_IP", "BAN", "KICK");
        if (actions.isEmpty() || !validActions.containsAll(actions)) {
            sender.sendMessage(Component.text("无效的措施。可用选项: BAN_IP, BAN, KICK，可同时指定多个。", NamedTextColor.RED));
            return true;
        }

        configManager.setActions(actions);
        sender.sendMessage(Component.text("已将处罚措施设置为: " + String.join(",", actions), NamedTextColor.GREEN));
        return true;
    }
    ...
    private boolean handleIpRangeCommand(CommandSender sender, String[] args) {
        if (args.length < 2) {
        ...
        List<String> ranges = new ArrayList<>(configManager.getBannedIpRanges());
        if (sub.equals("add")) {
            if (args.length < 3) {
            ...
            if (!configManager.isValidIpRange(range)) {
                sender.sendMessage(Component.text("IP 段格式无效，支持 x 通配符和 CIDR。", NamedTextColor.RED));
                return true;
            }
            if (ranges.stream().anyMatch(existing -> existing.equalsIgnoreCase(range))) {
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length == 1) {
            return Arrays.asList("reload", "blacklist", "action", "packetloss", "iprange").stream()
                    .filter(s -> s.startsWith(args[0].toLowerCase()))
                    .collect(Collectors.toList());
        }
        if (args.length == 2) {
            if (args[0].equalsIgnoreCase("blacklist") || args[0].equalsIgnoreCase("packetloss")
                    || args[0].equalsIgnoreCase("iprange")) {
                return Arrays.asList("add", "remove", "list").stream()
                        .filter(s -> s.startsWith(args[1].toLowerCase()))
                        .collect(Collectors.toList());
            }
            if (args[0].equalsIgnoreCase("action")) {
                return Arrays.asList("BAN_IP", "BAN", "KICK").stream()
                        .filter(s -> s.startsWith(args[1].toUpperCase()))
                        .collect(Collectors.toList());
            }
        }
        if (args.length == 3 && args[0].equalsIgnoreCase("packetloss")) {
            ...
            if (args[1].equalsIgnoreCase("remove")) {
                return packetLossManager.getPacketLossPlayers().stream()
        ...
        if (args.length == 3 && args[0].equalsIgnoreCase("iprange") && args[1].equalsIgnoreCase("remove")) {
